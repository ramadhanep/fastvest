...
<template>
  <UiActionSheet :open="!!holding" @close="emit('close')">
    ...
    <div class="px-5 pt-1 pb-5">
      <button
        type="button"
        class="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-destructive text-white h-12 text-sm font-semibold transition-colors ios-press cursor-pointer hover:bg-destructive/90 active:bg-destructive/80"
        @click="emit('confirm')"
      >
